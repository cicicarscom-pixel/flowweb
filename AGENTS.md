...
## ğŸš¨ Kritik Kural: Fonksiyon SahipliÄŸi ve Ä°simlendirme (31.08.2026)
...
| ledger-generate-schema | Åema Ã¼retimi |
...
## ğŸš¨ Kritik Kural: Deploy SÃ¼reci ve YasaklÄ± Fonksiyonlar
...
### [01.09.2026] Mobil ve Web ModÃ¼llerinde TasarÄ±m EÅŸitlemesi, CRM Entegrasyonu ve Bug Fix'ler
1. **Flow Mobil (React Native) - MÃ¼ÅŸteriler (CRM) ModÃ¼lÃ¼:** Web tarafÄ±ndaki "MÃ¼ÅŸteriler" mantÄ±ÄŸÄ± mobil tarafa Clean Architecture ile (domain/entities/Customer, ICustomerRepository, SupabaseCustomerRepository) eklendi. MÃ¼ÅŸteriler, Supabase Ã¼zerinden customers ve ppointments join'lenerek ekranda listelendi. MusterilerScreen.js oluÅŸturulup TabNavigator'a baÄŸlandÄ±.
2. **Flow Mobil - KÄ±rÄ±k Import ve Bundle Crash Ã‡Ã¶zÃ¼mleri:** WahaService.ts iÃ§indeki bozuk @infrastructure/api/supabaseClient importu dÃ¼zeltilerek Metro Bundler'Ä±n Ã§Ã¶kmesi (App.js bundling failed) giderildi. AyrÄ±ca OAuth Redirect Uri config ayarlarÄ± gÃ¼ncellenerek Supabase Whitelist sorunlarÄ± etrafÄ±ndan dolaÅŸÄ±ldÄ±.
3. **Flow Mobil - Master AI Toggle KaldÄ±rÄ±lmasÄ±:** BotYonetimiScreen.js'deki ana AI aÃ§/kapat ÅŸalteri UI Ã¼zerinden kaldÄ±rÄ±larak, alt platform (WhatsApp) ÅŸalterlerinin her zaman aktif gÃ¶rÃ¼nebilmesi saÄŸlandÄ±.
4. **Flow Web (Next.js) - Takvim Saat Dilimi Bug Fix:** RandevuClient.tsx'in kullandÄ±ÄŸÄ± sayfa seviyesindeki (page.tsx) 	oday deÄŸiÅŸkeni UTC olduÄŸu iÃ§in gece saatlerinde takvimi Ã¶nceki gÃ¼nde (Ã¶r: hala AÄŸustos) gÃ¶stermesine sebep oluyordu. Bu, yerel saat dilimi offset'i kullanÄ±larak dÃ¼zeltildi.
5. **Flow Web - Randevu EkranÄ± TasarÄ±mÄ±nÄ±n Mobile EÅŸitlenmesi:** Web'deki iki sÃ¼tunlu randevu takvimi ve yoÄŸunluk haritasÄ± dÃ¼zeni lex-direction: column ile tek sÃ¼tun yapÄ±ldÄ±. **Takvim** Ã¼stte, **GÃ¼nlÃ¼k YoÄŸunluk HaritasÄ± (MÃ¼saitlik)** ortada ve **Randevu Listesi** en altta olacak ÅŸekilde dikey olarak sÄ±ralandÄ±.
6. **Flow Web - Takvim Scroll UX Ä°yileÅŸtirmeleri:** Takvim ve YoÄŸunluk HaritasÄ± container'larÄ±na yatay kaydÄ±rma Ã§ubuklarÄ±nÄ± gizleyen CSS sÄ±nÄ±flarÄ± eklendi. overscroll-behavior-x: contain eklenerek saÄŸa-sola swipe yaparken tÃ¼m ekranÄ±n kaymasÄ± (swipe to go back veya page scroll) engellendi, native mobil hissi yaratÄ±ldÄ±.
7. **Flow Web - Ãœlke Listesi Dropdown Renk DÃ¼zeltmesi:** Profil ekranÄ±ndaki Ã¼lke, ÅŸehir, ilÃ§e <select> etiketlerindeki <option>'larÄ±n varsayÄ±lan beyaz/aÃ§Ä±k renk arka planlarÄ± #17151A olacak ÅŸekilde gÃ¼ncellenerek, Ã¼zerine gelen beyaz metinlerin okunamamasÄ± sorunu (koyu tema uyumsuzluÄŸu) Ã§Ã¶zÃ¼ldÃ¼.

---
## [07.09.2026] Zernio GÃ¶rÃ¼ntÃ¼ ve Medya Senkronizasyon Debug Logu
- **Sorun:** Zernio webhook'larÄ± Ã¼zerinden gelen mesaj ve yorumlarda (Gelen Kutusu) profil resimlerinin olmamasÄ± ve Sosyal Medya sayfasÄ±nda gÃ¶nderi gÃ¶rsellerinin "kÄ±rÄ±k resim" ikonu ÅŸeklinde Ã§Ä±kmasÄ±.
- **Bulgular:**
  1. comment.received webhook'unda post resim linki (post.imageUrl) bulunmasÄ±na raÄŸmen bu bilgi zernio-webhook tarafÄ±ndan okunup posts tablosuna aktarÄ±lmÄ±yordu.
  2. Yorum yapan kiÅŸinin avatarÄ± Zernio tarafÄ±ndan bu event'te zaten saÄŸlanmÄ±yor (dÃ¼zeltilemez dÄ±ÅŸ kÄ±sÄ±t).
  3. message.received/sent eventlerinde mesaj gÃ¶nderen profil resmi participantPicture veya sender.picture olarak geliyordu ancak veritabanÄ±ndaki conversations tablosunda bunu tutacak bir participant_picture kolonu bile yoktu!
  4. zernio-client iÃ§indeki sync-posts fonksiyonu sadece **yeni** gÃ¶nderileri insert yapÄ±yor, var olan gÃ¶nderilerin medya gÃ¼ncellemelerini es geÃ§iyordu. AyrÄ±ca medya linki olarak Zernio .blob linkleri (ki bunlar video olabiliyor) veya sÃ¼resi dolan CDN linkleri geliyordu.
  5. sync-posts'u upsert'e Ã§evirirken posts tablosundaki zernio_post_id kolonunda UNIQUE constraint (kÄ±sÄ±tlamasÄ±) olmadÄ±ÄŸÄ± iÃ§in Postgres onConflict iÅŸlemini reddediyordu.
  6. .blob linkleri .mp4 gibi bir uzantÄ± barÄ±ndÄ±rmadÄ±ÄŸÄ±ndan lowweb tarafÄ±ndaki <img src=...> etiketlerinde kÄ±rÄ±k ikon olarak gÃ¶rÃ¼nÃ¼yordu.
- **Ã‡Ã¶zÃ¼mler (Ledger reposu Ã¼zerinden):**
  1. 20260907000002_add_participant_picture_to_conversations.sql ve 20260907000003_add_unique_constraint_to_zernio_post_id.sql migration'larÄ± eklenip canlÄ± db'ye (
px supabase db push) basÄ±ldÄ±.
  2. zernio-webhook gÃ¼ncellenerek payload.post.imageUrl okumasÄ± yapÄ±lÄ±p posts tablosundaki media_urls alanÄ±na update atanmasÄ± saÄŸlandÄ±. AyrÄ±ca participant_picture okumasÄ± da eklenip conversations insert/update mantÄ±ÄŸÄ±na dÃ¢hil edildi.
  3. zernio-client/sync-posts fonksiyonu yeni gelen mappedPosts listesini tamamen upsert yapacak (varolanÄ± ezecek) ÅŸekilde refaktÃ¶r edildi. CDN baÄŸlantÄ±larÄ± blob'lara karÅŸÄ± Ã¶nceliklendirildi.
  4. Yeni edge function'lar canlÄ± ortama (
px supabase functions deploy) deploy edildi.
- **Ã‡Ã¶zÃ¼mler (Flowweb reposu Ã¼zerinden):**
  1. Frontend'de sosyal-medya/posts ve gelen-kutusu sayfalarÄ±ndaki resim <img> render kÄ±sÄ±mlarÄ±na bir regex eklendi: /\.(mp4|webm|ogg|mov|blob)(\?.*)?$/i || includes('blob'). 
  2. Video ve Zernio blob linkleri artÄ±k kÄ±rÄ±k ikonlu <img> yerine native HTML5 <video src=... muted playsInline> etiketiyle render ediliyor (otomatik ilk kareyi thumbnail olarak kullanÄ±yor) ve Ã¼zerine play ikonu yerleÅŸtiriliyor.
- **Teknik BorÃ§ (Sonraki AdÄ±mlar Ä°Ã§in UyarÄ±):** Frontend'deki regex / uzantÄ± kontrolÃ¼ Zernio'nun jenerik .blob uzantÄ±sÄ±ndan Ã¶tÃ¼rÃ¼ uzun vadede (blob bir resim postu gelirse) risklidir. En doÄŸru yÃ¶ntem posts tablosuna media_type kolonu ekleyip frontend'de buna gÃ¶re render kontrolÃ¼ yapmaktÄ±r (README.md'ye kaydedilmiÅŸtir).
---
## [16.09.2026] Zernio Senkronizasyon, UI Bug Fix'leri ve Veri KaybÄ± Ã–nlemleri
- **Bug 1 (UI Flicker):** Sosyal Medya sayfasÄ±nda (flowweb) her giriÅŸte hesap listesinin boÅŸalÄ±p Zernio'dan yanÄ±t dÃ¶nene kadar dÃ¶nÃ¼p durmasÄ± sorunu Ã§Ã¶zÃ¼ldÃ¼. `useEffect` iÃ§erisinde `fetchAccounts(false)` Ã§aÄŸrÄ±larak yerel verinin anÄ±nda render edilmesi, arka planda ise Zernio senkronizasyonunun sessizce devam etmesi saÄŸlandÄ±.
- **Bug 2 (Silinemeyen BaÄŸlantÄ±lar):** Zernio'da halihazÄ±rda kopmuÅŸ (needs_reconnection=true) olan hesaplarÄ±n, "Yeniden BaÄŸlan" kartlarÄ±ndaki Ã§Ã¶p kutusuna basÄ±ldÄ±ÄŸÄ±nda `disconnectAccount` API'sinden dÃ¶nen "404 Not Found" hatasÄ± nedeniyle yerelden silinememesi sorunu Ã§Ã¶zÃ¼ldÃ¼. ArtÄ±k "not found" durumunda yerel veritabanÄ±ndaki kayÄ±t gÃ¼venle siliniyor (ledger-repo / zernio-client).
- **Bug 3 (Kritik Veri KaybÄ± Ã–nlemi):** `zernio-client` iÃ§indeki `sync-posts` iÅŸleminin, boÅŸ/yeni bir profile baÄŸlandÄ±ÄŸÄ±nda Zernio'dan boÅŸ liste dÃ¶nmesi sonucu yerel veritabanÄ±ndaki tÃ¼m eski gÃ¶nderileri (sanki silinmiÅŸler gibi) kalÄ±cÄ± olarak silmesi engellendi. `postsList.length === 0` durumunda silme bloÄŸu tamamen atlanarak gÃ¼venlik kemeri eklendi.
- **Bug 4 (TikTok Caption KaybÄ± & ID UyuÅŸmazlÄ±ÄŸÄ±):** TikTok gÃ¶nderilerindeki "GÃ¶nderi detayÄ± bulunamadÄ±" hatasÄ±nÄ± Ã§Ã¶zmek iÃ§in webhook iÃ§ine REST fallback'i eklendi. Ancak webhook'un dÃ¶ndÃ¼rdÃ¼ÄŸÃ¼ TikTok'a ait native ID ile Zernio'nun REST API'sindeki Mongo tabanlÄ± ID eÅŸleÅŸmediÄŸi iÃ§in fallback Ã§alÄ±ÅŸmadÄ±. Ã‡Ã¶zÃ¼m olarak `zernio-client` iÃ§ine `ZERNIO_POST_PLATFORMS_RAW` logu eklendi; buradan TikTok'un gerÃ§ek native ID'sinin nasÄ±l geldiÄŸi tespit edilip ID eÅŸleÅŸtirme ve temizlik yapÄ±lacak.
