...
const startIdx = content.indexOf('{/* Manage Calendars Modal */}');
if (startIdx !== -1) {
    const endIdx = content.indexOf('<style dangerouslySetInnerHTML={{__html: `', startIdx);
    if (endIdx !== -1) {
        // Remove the badly injected block
...
const lastIdx = content.lastIndexOf('<style dangerouslySetInnerHTML={{__html: `');
if (lastIdx !== -1) {
    content = content.substring(0, lastIdx) + modals + content.substring(lastIdx + '<style dangerouslySetInnerHTML={{__html: `'.length);
}

fs.writeFileSync('src/app/(dashboard)/ai-asistan/randevu/RandevuClient.tsx', content, 'utf8');
console.log("Moved modal to the correct place!");
