...
file_path = 'src/app/(dashboard)/sosyal-medya/page.tsx'
with open(file_path, 'r', encoding='utf-8') as f:
    content = f.read()

# Split logic
# Find accounts.length > 0
start_idx = content.find('{accounts.length > 0 ? (')
end_idx = content.find(')}', content.find(')', content.find('NoConnectedAccounts'))+1) + 2
...
content = content[:start_idx] + new_block.strip() + content[end_idx:]
