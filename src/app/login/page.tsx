...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-[100dvh] bg-[#141319] text-white selection:bg-[#00A2FF]/30 selection:text-white font-sans flex relative">
      ...
      <div className="flex-1 relative z-10 flex flex-col items-center justify-center p-6 md:p-8 lg:p-12 overflow-y-auto">
        <div className="w-full max-w-[400px] animate-fade-in-up">
          {/* Mobile Logo */}
          <div className="flex lg:hidden items-center justify-center gap-2 mb-8">
            <img src="/logo.png" alt="Workigom Flow" className="h-20 w-auto object-contain" />
          </div>

          <div className="mb-6">
            <h2 className="text-[24px] lg:text-[28px] font-extrabold text-white mb-2">
              {mode === 'login' ? 'Hesabınıza giriş yapın' : 'Hesap Oluşturun'}
            </h2>
            <p className="text-[#8E95B3] text-[13px] lg:text-[14px]">
              {mode === 'login' ? 'İş akışlarınıza ulaşmak için giriş yapın.' : 'Workigom Flow dünyasına katılın.'}
            </p>
          </div>

          <button type="button" className="w-full flex items-center justify-center gap-3 bg-white text-[#07090E] font-bold py-3 px-4 rounded-xl hover:bg-gray-100 transition-all hover:scale-[1.02] shadow-[0_0_15px_rgba(255,255,255,0.1)] mb-5 relative overflow-hidden group">
            ...
            <span className="relative z-10 text-[14px]">Google ile {mode === 'login' ? 'Giriş Yap' : 'Kayıt Ol'}</span>
          ...
          <div className="flex items-center gap-4 mb-5">
            ...
            <span className="text-[#8E95B3] text-[11px] font-medium uppercase tracking-wider">veya e-posta ile</span>
            <div className="flex-1 h-px bg-white/10"></div>
          </div>
          ...
          <form action={formAction} className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-[12px] font-bold text-gray-300">E-posta Adresi</label>
              <input 
                type="email" 
                name="email"
                required
                placeholder="ornek@sirket.com" 
                className="w-full bg-[#0D1017] border border-[#232B45] text-white px-4 py-3 rounded-xl focus:outline-none focus:border-[#00A2FF] focus:ring-1 focus:ring-[#00A2FF]/50 transition-all placeholder-[#8E95B3]/50 text-[14px]"
              />
            </div>
            
            <div className="flex flex-col gap-1.5">
              <div className="flex justify-between items-center">
                <label className="text-[12px] font-bold text-gray-300">Şifre</label>
                {mode === 'login' && (
                  <a href="#" className="text-[11px] text-[#00A2FF] hover:text-white transition-colors font-medium">Şifremi unuttum</a>
                )}
              </div>
              <input 
                type="password" 
            ...
            <button 
              type="submit" 
              disabled={isPending}
              className={`w-full text-center bg-gradient-to-r from-[#00A2FF] to-[#B600F8] text-white font-bold py-3 px-4 rounded-xl mt-2 hover:shadow-[0_0_20px_rgba(0,162,255,0.4)] transition-all hover:scale-[1.02] relative overflow-hidden group ${isPending ? 'opacity-70 cursor-not-allowed' : ''}`}
            >
          ...
          <p className="text-center text-[13px] text-[#8E95B3] mt-6">
