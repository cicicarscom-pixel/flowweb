...
export default function AiUretimPage() {
    ...
    <div className="flex-1 flex flex-col h-full overflow-hidden text-on-surface">
      <div className="h-14 flex items-center justify-between px-5 shrink-0 border-b border-white/5">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-bold text-[#e5e2e3]">İçerik Üret & Paylaş</h1>
        </div>
        ...
        <div className="max-w-2xl mx-auto space-y-6">
          ...
          <div className="flex justify-center w-full relative">
            <div className="w-full aspect-square max-w-[350px] p-[3px] rounded-[24px] relative group overflow-hidden bg-white/5">
              {/* Fake Animated Border using Tailwind CSS conic-gradient or just simple CSS */}
          ...
          {/* YouTube Settings specific block */}
          {selectedPlatforms['youtube'] && (
            <div className="bg-[#1c1b1c]/50 rounded-xl p-4 border border-[#FF0000]/30">
              <div className="flex items-center mb-4">
                <i className="fa-brands fa-youtube text-[#FF0000] text-xl mr-2"></i>
                <span className="text-[#e5e2e3] text-sm font-semibold">YouTube</span>
              </div>
              
              <label className="block text-[#b9cacb] text-xs font-medium mb-1">title (optional)</label>
              <input type="text" placeholder="Custom title for your video..." className="w-full bg-[#0e0e0f]/50 border border-white/5 rounded-lg text-[#e5e2e3] text-sm px-3 py-2 mb-1 focus:outline-none focus:border-[#FF0000]/50" />
              <div className="text-[#b9cacb]/50 text-[10px] text-right mb-2">0/100</div>

              <label className="block text-[#b9cacb] text-xs font-medium mb-1">visibility</label>
              <div className="flex rounded-lg border border-white/5 mb-4 overflow-hidden">
                {['Public', 'Unlisted', 'Private'].map((opt) => (
                  <button key={opt} className={`flex-1 flex flex-col items-center py-2 border-r border-white/5 last:border-0 ${opt === 'Public' ? 'bg-[#2a2a2b]' : 'bg-[#0e0e0f]/50 hover:bg-[#1a1a1b]'}`}>
                    <span className={`text-[13px] ${opt === 'Public' ? 'text-white' : 'text-[#b9cacb]'}`}>{opt}</span>
                    <span className="text-[#b9cacb]/50 text-[10px]">{opt === 'Public' ? 'Anyone' : opt === 'Unlisted' ? 'Link only' : 'Only you'}</span>
                  </button>
                ))}
              </div>
            </div>
          ...
          {/* Facebook Settings specific block */}
          {selectedPlatforms['facebook'] && (
            <div className="bg-[#1c1b1c]/50 rounded-xl p-4 border border-[#1877F2]/30">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center">
                  <i className="fa-brands fa-facebook text-[#1877F2] text-xl mr-2"></i>
                  <span className="text-[#e5e2e3] text-sm font-semibold">Facebook</span>
                </div>
                <div className="flex rounded-lg border border-white/5 overflow-hidden">
                  {['Feed', 'Story', 'Reel'].map((opt) => (
                    <button key={opt} className={`px-2 py-1 text-[10px] border-r border-white/5 last:border-0 ${opt === 'Feed' ? 'bg-[#2a2a2b] text-white' : 'bg-[#0e0e0f]/50 text-[#b9cacb] hover:bg-[#1a1a1b]'}`}>
                      {opt}
                    </button>
                  ))}
                </div>
              ...
              <textarea placeholder="Drop any extra context or a CTA here." className="w-full bg-[#0e0e0f]/50 border border-white/5 rounded-lg text-[#e5e2e3] text-sm px-3 py-2 min-h-[60px] focus:outline-none focus:border-[#1877F2]/50 resize-none"></textarea>
            </div>
          )}
