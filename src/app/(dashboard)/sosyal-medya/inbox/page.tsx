...
export default function GelenKutusuPage() {
 ...
 {/* Sidebar (List) */}
 <div className="w-[350px] shrink-0 border-r border-white/10 flex flex-col /80 backdrop-blur-md">
 {/* Header */}
 ...
 <i className={`fa-brands fa-${conv.platform} text-[10px] text-on-surface`}></i>
 </div>
 </div>
 ...
 <h4 className="text-on-surface text-sm font-medium truncate pr-2">{conv.userName}</h4>
 <span className="text-on-surface-variant text-xs shrink-0">{conv.timestamp}</span>
 </div>
 <p className={`text-xs truncate ${conv.unreadCount > 0 ? "text-secondary font-medium" : "text-on-surface-variant"}`}>
 ...
 <div>
 <h3 className="text-on-surface text-sm font-semibold">{selectedConv.userName}</h3>
 ...
 <button className="w-9 h-9 rounded-full bg-white/5 flex items-center justify-center text-on-surface-variant hover:text-on-surface hover:bg-white/10 transition-colors">
 ...
 <span className="text-xs bg-white/5 text-on-surface-variant px-3 py-1 rounded-full border border-white/10">Bugün</span>
 </div>
 ...
 <div className={`text-[10px] mt-1 text-right ${msg.sender === "ai" ? "text-secondary/70" : "text-on-surface-variant"}`}>
 {msg.time}
 </div>
 </div>
 </div>
